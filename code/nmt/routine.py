    ...
    while True:
        ...
        for src_sents, tgt_sents, key in batch_iter(train_data, batch_size=train_batch_size, shuffle=True, sampling_multi=sampling_multi):
            ...
            if train_iter % valid_niter == 0:
                ...
                dev_ppl = model.evaluate_ppl(dev_data, batch_size=config.batch_size)
...
def train_encoder(model, train_data, dev_data, model_save_path, train_batch_size, valid_niter, log_every, max_epoch, lr, max_patience, max_num_trial, lr_decay, sampling_multi=1):
    ...
    while True:
        ...
        for src_sents, tgt_sents, key in batch_iter(train_data, batch_size=train_batch_size, shuffle=True, sampling_multi=sampling_multi):
            ...
            loss = model.encode_to_loss(src_sents, key=key)
...
def beam_search(model, test_data_src, key=None, max_step=None, replace=False):
    # was_training = model.training

    hypotheses = []
    for src_sent in tqdm(test_data_src, desc='Decoding', file=sys.stdout):
        example_hyps = model.beam_search(
            src_sent, max_step=max_step, replace=replace, key=key)
